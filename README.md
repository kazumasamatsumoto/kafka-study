...
**1. 初めての方**: [`GETTING_STARTED.md`](GETTING_STARTED.md)（30分）
  - 環境セットアップ
  - Kafkaの3つの基本概念（Producer、Kafka、Consumer）
  - 動作確認

**2. 学習を始める**: [`LEARNING_GUIDE.md`](LEARNING_GUIDE.md)（2-3日）
  - 7つのステップで体系的に学習
  - 各ステップにフローチャートとデータフロー付き
  - 実行コマンド → 期待される出力 → 確認方法

**3. 内部の仕組みを知る**: [`KAFKA_INTERNALS.md`](KAFKA_INTERNALS.md)（コラム・オプション）
  - Kafkaが生まれた背景と理由
  - なぜKafkaは高速なのか？（Zero-Copy、ログベース設計）
  - 内部アーキテクチャとレプリケーション
  - 他のメッセージングシステムとの違い
...
このプロジェクトのドキュメントは、学習の流れに沿って4つに整理されています：
...
### 3. 内部の仕組み（コラム・オプション）
- **[KAFKA_INTERNALS.md](KAFKA_INTERNALS.md)** - Kafkaの内部アーキテクチャ
  - Kafkaが生まれた背景（LinkedInのデータパイプライン問題）
  - なぜKafkaは高速なのか？（シーケンシャル書き込み、Zero-Copy、ページキャッシュ）
  - ログベース設計の仕組み
  - レプリケーションとPartitionの内部動作
  - 他のメッセージングシステム（RabbitMQ、Kinesis）との違い
...
### 📚 オプション: 内部の仕組み（`KAFKA_INTERNALS.md`）

Kafkaの内部アーキテクチャや、なぜ高速なのかを知りたい場合は、`KAFKA_INTERNALS.md`を参照してください：
- Kafkaが生まれた背景
- シーケンシャル書き込み、Zero-Copy、ページキャッシュ
- ログベース設計
- レプリケーションの仕組み
...
├── KAFKA_INTERNALS.md          # Kafka内部の仕組み（コラム・オプション）
├── README.md                   # このファイル
